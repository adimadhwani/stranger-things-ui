...
const LoginScreen = ({ onLogin }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (accessCode.toUpperCase() !== 'DEMOGORGON') {
    ...
    <div className="login-wrapper">
      ...
      <div className="login-container">
        ...
        <div className="login-footer">
          <p>WARNING: UNAUTHORIZED ACCESS IS A CLASS 1 FELONY</p>
          <small>DEPT. OF ENERGY // ID: HNL-85</small>
        </div>
      </div>
    </div>
  );
...
const StrangerThingsDashboard = () => {
  ...
  // Handle successful login from child component
    ...
    <div className={`dashboard-wrapper ${isUpsideDown ? 'upside-down' : ''}`}>
      ...
      <main className="container">
        ...
        <section className="hero">
          <h2>
            Welcome to Hawkins Lab:<br />
            <span style={{ color: 'var(--danger)' }}>API Coordination Training</span>
          </h2>
          <p>
            Eleven and Mike are separated across dimensions. Use your mastery of HTTP Methods 
            to pass items through the rift and coordinate their escape.
          </p>

          <div className="cards-container">
            <CharacterCard name="Eleven" items={['Eggos', 'Psychic Blast']} world="Upside Down" borderColor="#ef4444" />
            <div style={{ alignSelf: 'center' }}>
              <Activity size={48} color={isUpsideDown ? '#ef4444' : '#0F4C75'} />
            </div>
            <CharacterCard name="Mike" items={['Walkie-Talkie', 'Flashlight']} world="Hawkins" borderColor="#3b82f6" />
          </div>

          {/* Game Start Logic - HARDWARE REVEAL UI */}
          {!gameStarted ? (
            <button onClick={() => setGameStarted(true)} className="cta-button">
              START YOUR ESCAPE
            </button>
          ) : (
            <div className="hardware-container">
              ...
              {/* Top Red Neon Sign */}
              <div className="hardware-top-bezel">
                <div className="neon-red-text">GATEWAY PORTAL OPENED</div>
              </div>

              {/* Middle Section: Green Screen & Blue Button */}
              ...
              {/* Bottom LED Footer */}
              <div className="hardware-bottom-bezel">
                <div className="led-matrix-display">
                  <span className="led-text">Your Team ID is: {teamData.id}</span>
                </div>
              </div>
            </div>
          )}
        ...
        <section className="monitor-section">
          <h3 style={{ fontFamily: 'var(--font-heading)', fontSize: '2rem', marginBottom: '1rem' }}>
...
const CharacterCard = ({ name, items, world, borderColor }) => (
  <div className="char-card" style={{ borderColor: borderColor }}>
    ...
    {items.map(item => (
      <div key={item} style={{ background: 'rgba(0,0,0,0.5)', padding: '4px', fontSize: '0.8rem', marginBottom: '4px' }}>
        {item}
      </div>
    ))}
  </div>
